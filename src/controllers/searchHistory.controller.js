...
const saveLocSearched = asyncHandler(async (req, res) => {
  try {
    ...
    throw new apiErrorHandler(
      400,
      error.message || "failed to save searched location"
    );
  }
});
    ...
    return res
      ...
      .json(
        new apiResponseHandler(
          200,
          getLoc,
          "retrived saved locations successfully"
        )
      );
  } catch (error) {
...
// deleting the search history from mongoDB
const deleteLocSearched = asyncHandler(async (req, res) => {
  try {
    const deleteLoc = await SearchHistory.deleteMany();
    return res
      .status(200)
      .json(
        new apiResponseHandler(200, deleteLoc, "history deleted successfully")
      );
  } catch (error) {
    throw new apiErrorHandler(500, "failed to get delete history");
  }
});

export { saveLocSearched, getLocSearched, deleteLocSearched};
